...
def execute_run(runseed,phen,fno):
        #print('Phenotype',phen)
        D=50
        subsiz=0.0
        subset=0
        bo=[[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100],[-100,100]]
        opt=[100,200,300,400,500,600,700,800,900,1000,1100,1200,1300,1400,1500,1600,1700,1800,1900,2000,2100,2200,2300,2400,2500,2600,2700,2800,2900,3000]
        ...
        if phen.count(':')==6:
           ...
           solution, fitness,funerr = applyhigh(**params)
           ...
           solution, fitness,funerr = applylow(**params)
        
        
        return(abs(opt[findex]-fitness),funerr)
...
# 0. Check external parameters
    ...
    while num_evals <= max_evals:
        ...
        if mixing!='completenet':
            ...
            if rew=='separate':
               ...
               #print(pop_size)
            pops,fitnesses,num_evals=topology(pops,fitnesses,ns,pop_size,bounds,num_evals)
        ...
        if callback is not None:
            callback(**(locals()))
        if num_evals >= checkfes:
           funerr.append(abs(opt[fno-1]-min(fitness)))
           checkfes=checkfes+10000
    ...
    return population[best], fitness[best],funerr
...
def topology(pops,fitnesses,ns,new_pop_size,bounds,num_evals): 
        ...
        for j in range(ns):
            if i!=j:      
                ...
                if len(foundat[0])!=0: 
                     ...
                     fitnesses[i][foundat[0][0]]=apply_fitness(calcarray)
...
phen='200:60:separate:0.2:jade;shade;shade:number:distancecluster'
for z in range(1,31):
    seeds=[4963, 4599, 9589, 3812, 3196, 3803, 7967, 5348, 3074, 1803, 8797, 3737, 6014, 9670, 1945, 7009, 5749, 2600, 2652, 9392, 5309, 4600, 9819, 6027, 6569, 5477, 1506, 4211, 5848, 3350, 1705, 5366, 4837, 9931, 4457, 2637, 6016, 5850, 1250, 7309, 6119, 2936, 8976, 3991, 4484, 3558, 2615, 3896, 8643, 4965, 9509]
    ...
    for run in range(51):
        results.append(pool.apply_async(execute_run, args=(seeds[run],phen,z,)))
    ...
    for i in ret:
        ...
        funerrs.append(i[1])
    pool.close()
    f1=open("1/ErrorsF"+str(z)+".txt","w")
    for i in errors:
       f1.write(str(i))
       f1.write('\n')
    f1.write("Minimum error among the 51 runs:")
    f1.write(str(min(errors)))
    f1.write("\nMaximum error among the 51 runs:")
    f1.write(str(max(errors)))
    f1.write("\nMean error:")
    f1.write(str(statistics.mean(errors)))
    f1.write("\nMedian of errors:")
    f1.write(str(statistics.median(errors)))
    f1.write("\nStd deviation of errors:")
    f1.write(str(statistics.pstdev(errors)))
    f1.close()
    f2=open("1/ErrorsAt10000fesF"+str(z)+".txt","w")
    for i in funerrs:
       f2.write(str(i))
       f2.write('\n')
    f2.close()
    ...
    for j in range(50):
        ...
        for i in range(51):
             s=s+funerrs[i][j]
        avfunerr.append(s/51.0)
    f3=open("1/AverageFunctionErrorsAt10000gensF"+str(z)+".txt","w")
    for i in avfunerr:
        f3.write(str(i))
        f3.write('\n')
    f3.close()
