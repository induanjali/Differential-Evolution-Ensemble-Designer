...
def evolutionary_algorithm(grammar = "", exp_name = "", eval_func = ""):
    ...
    random.seed(SEED)
    stats = ""
    progress_report = "Generation,Best,Mean,Tree depth,Phenotype\n"
    ...
    for it in range(it, NUMBER_OF_ITERATIONS):
        ...
        if 'test_error' in best['other_info']:
            ...
            stats = "" + str(it) + "," + str(best['fitness']) + "," + str((float(sum([ind['fitness'] for ind in population])) / float(POPULATION_SIZE))) + "," + str(best['tree_depth']) + "," + str(best['phenotype'])
